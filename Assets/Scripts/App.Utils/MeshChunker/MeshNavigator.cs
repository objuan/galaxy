...
namespace brickgame
{
    ...
    public class NavWaypoint
    {
        ...
        public Vector3 PositionW => nav.GetFacePositionW(cell, face);
        ...
        {
            return cell == node.cell && face == node.face;
        }
    }
    ...
    public class Node
    {
        ...
        public Vector3 Position(MeshNavigator nav) => nav.GetFacePositionW(point.cell, point.face);
    ...
    public class NavPathBuilder
    {
        ...
        {
            return nav.GetFaceNear(n.point.cell, n.point.face).Select(X => new Node(X, 0) { }).ToList();
        }
        ...
        public NavPath Compute(MeshNavigator nav)
        {
            ...
            navPath[navPath.Count - 1] = new NavWaypoint(nav, nav.target_cell, nav.target_face, nav.target_dir);
    ...
    public class MeshNavigator
    {
        ...
        MeshWorld world;
        public MeshChunkCell pos;
        public MeshFace face;
        public MeshDirection dir;

        public MeshChunkCell target_cell;
            ...
            for (int i = 1; i <= 6; i++)
            {
                if (IsDirectionValid(face, (MeshDirection)i))
                {
                    ...
                    if (world.IsValid(p) && world.IsFull(p))
                    {
                        ...
                        waypoint = new NavWaypoint(this) { dir = (MeshDirection)face, cell = c, face = opposite[i] }; // DIR = prev
                    }
                    else
                    {
                        ...
                        if (world.IsValid(p) && world.IsFull(p))
                        {
                            ...
                            var c = world.GetCell(p);
                            if (c.isVisible)
                                waypoint =new NavWaypoint(this) { dir = (MeshDirection)i, cell = c, face = face };
                        }
                        else
                        {
                            // o fuori o dentro ma senza full. resto io 
                          
                            waypoint = new NavWaypoint(this) { dir = (MeshDirection)opposite[(int)face], cell = cell, face = (MeshFace)i }; //dir = prev oppusto
                        }
                    }
                    if (waypoint!=null)
                        steps.Add(waypoint);
        ...
        {
            return GetFaceRelativePosition(posW, pos, face, ref facePositionW, ref isUP);
        ...
        {
            SetTarget(wayPoint.cell, wayPoint.face, wayPoint.dir);
        }
        ...
        public void SetTarget(MeshChunkCell cell, MeshFace face, MeshDirection dir)
        {
            ...
            // find shortest path
            var b = new NavPathBuilder() { root = new NavWaypoint(this,this.pos, this.face,this.dir), target = new NavWaypoint(this,target_cell, target_face, dir), world = world };
